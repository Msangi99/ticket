...
@section('content')
    <div class="max-w-2xl mx-auto">
        <div
            class="bg-white/80 dark:bg-slate-900/70 backdrop-blur-lg shadow-xl rounded-2xl p-6 md:p-8 transition-all duration-300 hover:shadow-2xl">
            ...
            @if (session('status'))
                <div
                    class="mb-5 rounded-lg border border-green-200/60 dark:border-green-800/50 bg-green-50/70 dark:bg-green-900/30 text-green-800 dark:text-green-100 px-4 py-3">
            ...
            @if (session('error'))
                <div
                    class="mb-5 rounded-lg border border-red-200/60 dark:border-red-800/50 bg-red-50/70 dark:bg-red-900/30 text-red-800 dark:text-red-100 px-4 py-3">
            ...
            @if ($errors->any())
                <div
                    class="mb-5 rounded-lg border border-amber-200/60 dark:border-amber-800/50 bg-amber-50/70 dark:bg-amber-900/30 text-amber-800 dark:text-amber-100 px-4 py-3">
            ...
            @if (!Auth::user()->two_factor_secret)
                {{-- Not enabled state --}}
                <div
                    class="rounded-xl border border-gray-200 dark:border-slate-700 bg-gray-50/60 dark:bg-slate-800/50 p-5 mb-6">
                    <h5 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-1">2FA is Disabled</h5>
                    <p class="text-sm text-gray-600 dark:text-gray-400">
                        Turn on two-factor authentication to require a one-time code from an authenticator app when you sign
                        in.
                    </p>
                </div>
                ...
                <form method="POST" action="{{ route('two-factor.enable') }}" class="mt-4">
                    ...
                    <button type="submit"
                        ...
                        <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                            <path
                                d="M10 2a6 6 0 016 6v1h1a1 1 0 110 2h-1v1a6 6 0 11-12 0V11H3a1 1 0 110-2h1V8a6 6 0 016-6zm0 2a4 4 0 00-4 4v1h8V8a4 4 0 00-4-4z" />
                        </svg>
                        Enable Two-Factor
                    </button>
                </form>
            @else
                {{-- Enabled state --}}
                <div
                    class="rounded-xl border border-emerald-200 dark:border-emerald-800 bg-emerald-50/60 dark:bg-emerald-900/30 p-5 mb-6">
                ...
                @if (session('status') == 'two-factor-authentication-enabled')
                    <div
                        class="rounded-lg border border-blue-200 dark:border-blue-800 bg-blue-50/70 dark:bg-blue-900/30 text-blue-900 dark:text-blue-100 px-4 py-3 mb-6">
                        Two-factor authentication is now enabled. Scan the QR code with your authenticator app (e.g., Google
                        Authenticator, Authy).
                    </div>
                @endif
                ...
                @if (Auth::user()->two_factor_secret == null)
                    <div class="mb-8">
                        ...
                        <div
                            class="rounded-xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-4 inline-block">
                            {!! $qrCodeSvg !!}
                        </div>
                ...
                <div class="mb-8">
                    <form method="POST" action="{{ route('two-factor.confirm') }}" class="space-y-4">
                        ...
                        <div>
                            <label for="code"
                                class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">One-Time
                                Code</label>
                        ...
                        <button type="submit"
                            ...
                            <svg class="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
                                <path
                                    d="M9 12l2 2 4-4 1.5 1.5L11 16l-3.5-3.5L9 12zm3-9C6.48 3 2 7.48 2 13s4.48 10 10 10 10-4.48 10-10S17.52 3 12 3z" />
                            </svg>
                            Confirm
                        </button>
                    </form>
                ...
                {{-- Recovery codes --}}
                @if (Auth::user()->two_factor_recovery_codes == null)
                    <div class="mb-8">
                            ...
                            <form method="POST" action="{{ route('two-factor.recovery-codes') }}">
                                ...
                                <button type="submit"
                                    ...
                                    <svg class="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
                                        <path d="M5 4h14v2H5zm0 4h14v2H5zm0 4h9v2H5z" />
                                    </svg>
                                    Generate New Codes
                                </button>
                            </form>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                            @foreach (json_decode(decrypt(Auth::user()->two_factor_recovery_codes), true) as $code)
                                <div
                                    class="font-mono text-sm tracking-wider rounded-md border border-gray-200 dark:border-slate-700 bg-gray-50/60 dark:bg-slate-800 px-3 py-2 text-gray-800 dark:text-gray-100">
                                    {{ $code }}
                                </div>
                            @endforeach
                        </div>
                ...
                <!-- <div class="pt-4 border-t border-gray-200 dark:border-slate-700">
                    <form method="POST" action="{{ route('two-factor.disable') }}">
                        ...
                        <button type="submit"
                            ...
                            <svg class="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
                                <path d="M12 2a10 10 0 1010 10A10.011 10.011 0 0012 2zm5 11H7v-2h10z" />
                            </svg>
                            Disable Two-Factor
                        </button>
                    </form>
