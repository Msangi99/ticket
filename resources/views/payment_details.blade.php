    ...
    <div class="max-w-6xl mx-auto">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
            ...
            <div class="text-sm text-white bg-red-600 px-4 py-2 rounded-lg shadow-sm flex items-center">
                ...
                <span>{{ __('all.your_session_expires_in') }} <span id="minutes">06</span> {{ __('all.mins') }} <span id="seconds">40</span> {{ __('all.secs') }}</span>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-md overflow-hidden">
                    <div class="p-6">
                        <h3 class="text-lg font-semibold text-gray-800 mb-2">{{ __('all.contact_details') }}</h3>
                        <p class="text-sm text-gray-600 mb-4">{{ __('all.fill_traveler_details') }}</p>
                        
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <!-- Country Code -->
                            <div>
                                <label for="countrycode" class="block text-sm font-medium text-gray-700 mb-1">{{ __('all.country_code') }}</label>
                            ...
                            <!-- Mobile Number -->
                            <div>
                                <label for="contactNumber" class="block text-sm font-medium text-gray-700 mb-1">{{ __('all.mobile_number') }}</label>
                            ...
                            <div class="md:col-span-2">
                                <label for="contactEmail" class="block text-sm font-medium text-gray-700 mb-1">{{ __('all.email_address') }}</label>
                ...
                <div class="bg-white rounded-xl shadow-md overflow-hidden">
                    <div class="p-6">
                        <h3 class="text-lg font-semibold text-gray-800 mb-4">{{ __('all.payment_options') }}</h3>
                        
                        <div class="flex flex-col md:flex-row gap-6">
                            <!-- Payment Methods Sidebar -->
                            <div class="md:w-1/3">
                                <div class="space-y-2" role="tablist" aria-label="{{ __('all.payment_methods') }}">
                                    ...
                                    <button type="button" class="w-full text-left px-4 py-3 rounded-lg hover:bg-gray-100 text-blue-700"
                                        ...
                                        <form id="tigo" action="{{ route('verify') }}" method="POST">
                                            ...
                                            <div class="space-y-4">
                                                <div class="p-4 bg-blue-50 rounded-lg">
                                                    <p class="text-sm text-gray-700 mb-1">{{ __('all.session_expiry_warning') }}</p>
                                                    <p class="text-lg font-bold text-green-600">{{ __('all.total') }} {{ $currency }}. {{ convert_money($price + $fees) }}</p>
                                                </div>
                                                
                                                <p class="text-gray-700">{{ __('all.enter_yas_mobile_number') }}</p>
                                                
                                                <div>
                                                    <label for="paymentContact" class="block text-sm font-medium text-gray-700 mb-1">{{ __('all.mobile_number') }}</label>
                                                    <input type="text" name="payment_contact" id="paymentContact" maxlength="10"
                                                        ...
                                                        placeholder="{{ __('all.connected_mobile_number') }}" required>
                                                </div>
                                                ...
                                                <div class="flex items-start">
                                                    ...
                                                    <div class="ml-3 text-sm">
                                                        <label for="payment_term_0" class="font-medium text-gray-700">{{ __('all.i_accept') }} <a href="{{ route('ticket.purchase') }}" class="text-blue-600 hover:text-blue-500">{{ __('all.terms_and_conditions') }}</a></label>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="bg-white rounded-xl shadow-md overflow-hidden mt-4">
                                                    <div class="p-4">
                                                        <h4 class="text-md font-semibold text-gray-800 mb-3">
                                                            <i class="fas fa-receipt mr-2 text-blue-500"></i> {{ __('all.order_summary') }}
                                                        </h4>
                                                        <div class="space-y-2">
                                                            <div class="flex justify-between">
                                                                <span class="text-sm text-gray-600">{{ __('all.discount') }}</span>
                                                                <span class="text-sm font-medium text-gray-500">{{ __('all.currency_prefix_tzs') }} {{ number_format($dis, 2) }}</span>
                                                            </div>
                                                            @if(isset($ins) && $ins > 0)
                                                            <div class="flex justify-between">
                                                                <span class="text-sm text-gray-600">{{ __('all.insurance') }}</span>
                                                                <span class="text-sm font-medium text-gray-500">{{ __('all.currency_prefix_tzs') }} {{ number_format($ins) }}</span>
                                                            </div>
                                                            @endif
                                                            <div class="flex justify-between">
                                                                <span class="text-sm text-gray-600">{{ __('all.system_charge') }}</span>
                                                                <span class="text-sm font-medium text-gray-500">{{ __('all.currency_prefix_tzs') }} {{ convert_money($fees) }}</span>
                                                            </div>
                                                            <div class="flex justify-between">
                                                                <span class="text-sm text-gray-600">{{ __('all.bus_fare') }}</span>
                                                                <span class="text-sm font-medium text-gray-500">{{ __('all.currency_prefix_tzs') }} {{ convert_money($price - $ins) }}</span>
                                                            </div>
                                                ...
                                                <button type="submit"
                                                    ...
                                                    <i class="fas fa-lock mr-2"></i> {{ __('all.proceed_to_pay') }}
                                                </button>
                                            </div>
                                        </form>
                                    </div>
                                    ...
                                    <div id="tab2" class="tab-pane" role="tabpanel" aria-labelledby="tab2-btn">
                                        <form id="dpo" action="{{ route('verify') }}" method="POST">
                                            ...
                                            <div class="space-y-4">
                                                ...
                                                <div>
                                                    <label for="dpo_amount" class="block text-sm font-medium text-gray-700 mb-1">{{ __('all.amount') }}</label>
                                                ...
                                                <div class="flex items-start">
                                                    ...
                                                    <div class="ml-3 text-sm">
                                                        <label for="dpo_terms" class="font-medium text-gray-700">{{ __('all.i_accept') }} <a href="{{ route('ticket.purchase') }}" class="text-blue-600 hover:text-blue-500">{{ __('all.terms_and_conditions') }}</a></label>
                                                    </div>
                                                </div>
